...
@dataclass(frozen=True)
class SeedreamTaskInput:
    """One per output image: 1-5 reference images + prompt + ratio."""

    reference_images_b64: list[str]
    prompt: str
    aspect_ratio: str  # one of seedream's enum, e.g. ``traditional_3_4``
    seed: int | None = None
...
class Generator:
    ...
    async def run_seedream_batch(
        self,
        *,
        tasks: list[SeedreamTaskInput],
        on_progress: ProgressCb | None = None,
    ) -> GenerationResult:
        """Run one Seedream 4.5 Edit task per ``tasks`` entry, in parallel.

        Each entry produces exactly one output image. Same key-rotation /
        retry semantics as :py:meth:`run_batch` (auth -> blacklist,
        quota -> cooldown, transient -> retry on a different key).
        """
        n = len(tasks)
        progresses = [_TaskProgress() for _ in range(n)]
        progress_lock = asyncio.Lock()
            ...
            while attempt < self._max_attempts:
                ...
                try:
                    async with httpx.AsyncClient(
                        ...
                        client = FreepikImageClient(api_key=key, client=http)
                        task_id = await client.create_seedream_edit_task(
                            prompt=task.prompt,
                            reference_images_b64_or_url=task.reference_images_b64,
                    ...
                    self._pool.report_failure(key, permanent=True)
                    last_exc = exc
                    logger.warning("Seedream task #%d auth gagal, rotasi key. %s", idx + 1, exc)
                    continue
                except QuotaError as exc:
                    self._pool.report_failure(key, permanent=False, cooldown_seconds=120.0)
                    last_exc = exc
                    logger.warning("Seedream task #%d kuota/rate, rotasi key. %s", idx + 1, exc)
                    continue
                except TransientError as exc:
                    self._pool.report_failure(key, permanent=False, cooldown_seconds=10.0)
                    last_exc = exc
                    logger.warning("Seedream task #%d transient, retry. %s", idx + 1, exc)
                    await asyncio.sleep(min(2.0 * attempt, 8.0))
                    continue
                except GenerationError as exc:
                    last_exc = exc
                    logger.warning("Seedream task #%d generation error: %s", idx + 1, exc)
                    break
            raise last_exc or GenerationError(f"Seedream task #{idx + 1} gagal tanpa exception")
